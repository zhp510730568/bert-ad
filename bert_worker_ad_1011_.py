...
if __name__=='__main__':
  ...
  checkpoint_path='model/ad/model_pretrain_1011_/model.ckpt-79365'
  graph_tmp_dir='model/ad/tmp/'

  configuration=Configuration(fp16, bert_config, checkpoint_path,graph_tmp_dir, 100)

  graph_path, bert_config = optimize_graph(configuration)
  worker = BertWorker(0, graph_path, configuration)
  
  with open("../ad_data/new_eval_ins2/v1011/lm_validset_0805.tsv", "r") as f:
    for line in f:
      row=line.split(" ")
      title = row[-1].strip()
     
      feature = get_example(tokenizer, text_a=title, text_b=None, max_seq_length=100)
      tag, score = worker.predict(feature)
      print('%f\t%s' % (score[0][1], line.strip()))
