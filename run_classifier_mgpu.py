...
class DataProcessor(object):
  ...
  def _read_tab_txt(cls, input_file, split_count=2):
    """Reads a tab separated value file."""
    with tf.gfile.Open(input_file, "r") as f:
      lines=[]
      for line in f:
...
class ToutiaoProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tab_txt(os.path.join(data_dir, "train_0705.tsv"), 2), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tab_txt(os.path.join(data_dir, "test_0705.tsv"), 2), "dev")

  def get_labels(self):
    """See base class."""
    labels="news_story,stock,news_house,news_finance,news_edu,news_car,news_travel,news_world,news_game,news_agriculture,news_sports,news_tech,news_military,news_entertainment,news_culture"
    return [label for label in labels.split(',')]
...
class TitleAuditProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "train_all_0531.txt")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "test_all_0531.txt")), "dev")
...
class AdLmProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "train_0620.txt")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "eval_0620.txt")), "dev")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      if set_type=='dev':
        ...
        test_b = tokenization.convert_to_unicode(line[3])
        label=tokenization.convert_to_unicode(line[0])
      else:
        text_a = tokenization.convert_to_unicode(line[2])
        test_b = tokenization.convert_to_unicode(line[3])
      ...
      example=InputExample(guid=guid, text_a=text_a, text_b=test_b, label=label)
...
class HighLevelProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "train_without_content_0515.txt")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, "test_without_content_0515.txt")), "dev")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      title = tokenization.convert_to_unicode(line[3].lower())
      label = line[2]
      examples.append(
        InputExample(guid=guid, text_a=title, text_b=None, label=label))
...
class DocClassifyProcessor(DataProcessor):
  """Processor for the CoLA data set (GLUE version)."""
  ...
  def get_labels(self):
    """See base class."""
    return ["beauty", "astro", "history", "food", "digital", "game", "science", "health", "baby", "houseliving", "ent", "edu", "pet", "tech", "finance", "mil", "agriculture", "sports", "travel", "house"]
...
def convert_examples_to_features(examples, label_list, max_seq_length,
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    if example.text_b:
      ...
      if hasattr(example, 'start_position'):
        start_position=getattr(example, 'start_position')
        current_index=0
        tokens_a_length=len(tokens_a)
        tokens_b_tmp=[]
        masked_count=0
        for index, token in enumerate(tokens_b):
          if token in sentence_delimeter:
            #tokens_b_tmp.append(token)
            current_index+=1
            continue
          if start_position==current_index:
            if token=='[UNK]' and masked_count < tokens_a_length:
              tokens_b_tmp.append('[MASK]')
              masked_count+=1
          else:
            tokens_b_tmp.append(token)
