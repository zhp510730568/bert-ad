...
def clip_by_global_norm_summary(t_list, clip_norm, norm_name, variables):
  ...
  all_are_finite = tf.reduce_all([tf.reduce_all(tf.is_finite(g)) for g in t_list])

  # clip
  clipped_t_list, tf_norm = tf.clip_by_global_norm(t_list, clip_norm,use_norm=tf.cond(
                all_are_finite,
                lambda: tf.global_norm(t_list),
...
def clip_grads(grads, all_clip_norm_val, do_summaries, global_step):
  ...
  def _clip_norms(grad_and_vars, val, name):
    ...
    if do_summaries:
      # fix bug: Nan or Inf
      #all_are_finite = tf.reduce_all([tf.reduce_all(tf.is_finite(g)) for g in grad_tensors])
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    next_sentence_labels_list = tf.split(features["next_sentence_labels"], num_or_size_splits=num_gpus, axis=0)
        ...
        with tf.device('/gpu:%d' % index):
          ...
          (next_sentence_loss, next_sentence_example_loss,
           next_sentence_log_probs) = get_next_sentence_output(
               bert_config, model.get_pooled_output(), next_sentence_labels_list[index])
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      eval_metrics = (metric_fn, [
          ...
          masked_lm_weights_list[0], next_sentence_example_loss,
          next_sentence_log_probs, next_sentence_labels_list[0]
      ])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def main(_):
  ...
  for input_pattern in FLAGS.input_file.split(","):
    file_list = tf.gfile.ListDirectory(input_pattern)
    for file in file_list:
      if file.endswith('.tfrecord'):
        file_path = os.path.join(FLAGS.input_file, file)
        input_files.extend(tf.gfile.Glob(file_path))
