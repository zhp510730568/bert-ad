...
def main(_):
  ...
  checkpoint_path='./model/ad/model_pretrain_0916/model.ckpt-203398'
  ...
  data_path='/data1/zhangpengpeng/ad_data/eval_ins2_20190809'

  start=time.time()
  for no in range(100):
    suffix=10000 + FLAGS.task_num * no + FLAGS.task_index
    slice_path=os.path.join(data_path, 'slice_gz_%s' % str(suffix)[1:])
    slice_output_path=os.path.join(data_path, 'slice_output_gz_%s' % str(suffix)[1:])
